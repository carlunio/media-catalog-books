...
from src.backend.services import snapshots  # noqa: E402
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Snapshot management for media-catalog-books"
    )
    ...
    publish_parser.add_argument(
        "--notes", default=None, help="Optional notes stored in the manifest"
    )
    publish_parser.add_argument(
        "--no-cleanup",
        action="store_true",
        help="Skip automatic cleanup after publishing",
    )

    import_parser = subparsers.add_parser(
        "import", help="Import a verified snapshot into the local DB"
    )
    import_parser.add_argument("snapshot_id", help="Snapshot ID to import")
    import_parser.add_argument(
        "--confirm", action="store_true", help="Confirm replacing the local DB"
    )

    subparsers.add_parser(
        "cleanup", help="Delete old snapshots according to retention policy"
    )
